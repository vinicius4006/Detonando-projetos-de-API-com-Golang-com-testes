...
func TestSaveOK(t *testing.T) {
	postUrl := "http://localhost:4440/api/note"
	var jsonData = []byte(`{
	...
	if error != nil {
		log.Panicf("Can't save \n%v", error)
	}
	defer response.Body.Close()

	if response.StatusCode != http.StatusOK {
		t.Fatalf("TestSaveOK should return no error")
	}
	...
	if err != nil {
		t.Fatalf("TestSaveOK should return a valid json %v", request)
	}
